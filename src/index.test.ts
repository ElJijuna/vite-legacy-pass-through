...
import { DEFAULT_EXCLUDE_EXTENSIONS, legacyPassThrough } from './index';
...
describe('legacyPassThrough', () => {
  describe('validation', () => {
    ...
    it('throws when called with default options', () => {
      expect(() => legacyPassThrough()).toThrow('The "libs" option must be a non-empty array');
    });
  ...
  describe('resolveId', () => {
    ...
    it('handles multiple libs', () => {
      ...
      expect(resolveId.call({} as never, 'lib-a/index', undefined, opts)).toEqual({
        id: 'lib-a/index',
        external: true,
      });
      expect(resolveId.call({} as never, 'lib-b/utils', undefined, opts)).toEqual({
        id: 'lib-b/utils',
        external: true,
      });
      expect(resolveId.call({} as never, 'lib-c/foo/bar', undefined, opts)).toEqual({
        id: 'lib-c/foo/bar',
        external: true,
      });
  ...
  describe('excludeExtensions', () => {
    ...
    it('uses custom excludeExtensions instead of default', () => {
      ...
      expect(resolveId.call({} as never, 'my-lib/assets/icon.svg', undefined, opts)).toEqual({
    ...
    it('does not skip excluded extension from a non-matching lib', () => {
      ...
      expect(
        resolveId.call({} as never, 'other-lib/styles/button.css', undefined, opts),
      ).toBeNull();
    });
  });
  ...
  describe('showLog', () => {
    ...
    it('logs when showLog is true and source matches', () => {
      ...
      void resolveId.call({} as never, 'my-lib/utils', undefined, opts);
    ...
    it('does not log when source does not match', () => {
      ...
      void resolveId.call({} as never, 'other/utils', undefined, opts);
      expect(console.log).not.toHaveBeenCalled();
    });
  });
});
